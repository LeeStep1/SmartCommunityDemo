...
import cn.bit.framework.data.jdbc.DBRuntimeException;
import cn.bit.framework.data.jdbc.IDaoSupport;
import cn.bit.framework.data.jdbc.ObjectNotFoundException;
import cn.bit.framework.utils.ReflectionUtil;
...
public class JdbcDaoSupport implements IDaoSupport {
	...
	public int getLastId(String table) {
		...
		} else if (AppConfig.getInstance().getDbType().equals("2")) {
			int result = 0;
			result = queryForInt("SELECT s_" + table + ".currval as id from DUAL");
			return result;
		} else if (AppConfig.getInstance().getDbType().equals("3")) {
			int result = 0;
	...
	public void update(String table, Map fields, String where) {
		...
		if ("3".equals(AppConfig.getInstance().getDbType())) { // SQLServer主键不允许更新，执行update前先删除
			try {
//				where = where.replaceAll(" ", "");
	...
	public String buildPageSql(String sql, int page, int pageSize) {
		...
		String db_type = AppConfig.getInstance().getDbType();
		if (db_type.equals("1")) {
			db_type = "mysql";
