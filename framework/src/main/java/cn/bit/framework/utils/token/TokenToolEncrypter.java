package cn.bit.framework.utils.token;

import cn.bit.framework.exceptions.BizException;
import cn.bit.framework.utils.CommonUtils;
...
public class TokenToolEncrypter implements TokenBaseInter {
    ...
    public String productToken() {
        try {
            return this.encrypt(CommonUtils.getMACAddress().hashCode() + "-" + Long.valueOf(System.currentTimeMillis()).hashCode() + "-" + StrUtil
                    .get32UUID().hashCode() + "-" + key.hashCode());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
