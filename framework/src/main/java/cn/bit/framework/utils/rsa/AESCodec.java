...
public class AESCodec {
    ...
    public static void main(String[] args) throws Exception {
        byte[] key = DigestUtils.sha1("xxxxxxxx");
        for (byte s : key){
            System.err.print(s + " ");
        }
        ...
        bb.flip();
        byte[] aesKey = bb.array();
        for (byte s : aesKey){
            System.err.print(s + " ");
        }
        bb.clear();
        System.err.println();
        byte[] data = AESCodec.encrypt("你大爷".getBytes(), aesKey);
        System.err.println(new String(AESCodec.decrypt(data, aesKey)));
    }
