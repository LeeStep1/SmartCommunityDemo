package cn.bit.framework.utils.string;
...
public class StringUtil extends StringUtilParent {
     ...
     * 字符串脱敏
     *
     * @param firstIndex //前面保留几位
     * @param lastIndex  //后面保留几位
     * @param org
     * @return
     */
    public static String desensitize(String org, int firstIndex, int lastIndex) {//脱敏
        if (isNotNull(org) && org.length() > (firstIndex + lastIndex)) {//手机号脱敏作为昵称
            char[] chars = org.toCharArray();
            for (int i = firstIndex; i < org.length() - lastIndex; i++) {
                chars[i] = '*';
            }
    ...
    /**
     * subtotal 是放大了10000倍的
     * 计算实际缴费金额，Long存储 放大100倍
     *
     * @param subtotal
     * @return
     */
    public static Long calculateTotalAmount(Long subtotal) {
        if (subtotal == 0) {//无效金额
            return subtotal;
        }
        String totalStr = subtotal.toString();
        if (totalStr.length() < 2) {//至少要有2位数
            totalStr = freemarker.template.utility.StringUtil.leftPad(totalStr, 2, "0");
        }
        String str = totalStr.substring(totalStr.length() - 2);//截取最后2位
        long totalAmount = subtotal / 100;

        if (Integer.parseInt(str) >= 50) {//四舍五入
            totalAmount += 1;
        }
        return totalAmount;
    }
     ...
     * @return
     */
    public static String makePriceKeep2Decimal(Long price) {
        if (price == null || price == 0) {
            return "0";
        }
        String totalAmount = leftPadWithBytes(price.toString(), 3, '0', "utf8");
