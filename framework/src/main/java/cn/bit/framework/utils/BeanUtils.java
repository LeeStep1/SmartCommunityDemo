...
public class BeanUtils {
     ...
     */
    public static void copyPropertiesAndIgnored(Object source, Object target, String... ignoredProperties) {

        String[] filteredProperties = Arrays.asList(getPropertyNames(target)).stream().filter(property ->
                !ArrayUtils.contains(ignoredProperties, property)
        ).collect(Collectors.toList()).toArray(new String[]{});

        copyProperties(source, target, filteredProperties);
    }
     ...
     * @return
     */
    public static <E> E filteProperties(E target, String... properties) {
        if (target == null)
            return null;
        for (String property : properties) {
            invokeSetter(target, property, null, false, false);
        }
        return target;
    }
